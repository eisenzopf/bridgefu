...
resource "aws_cloudformation_stack" "recipe" {
  ...
  lifecycle {
    precondition {
      ...
      error_message = "parameters is missing one or more required Starter Production CloudFormation inputs."
    }
    ...
    precondition {
      condition     = var.parameters["RuntimeProfile"] == "Starter"
      error_message = "The Starter module requires RuntimeProfile=Starter."
    }
  }
}
