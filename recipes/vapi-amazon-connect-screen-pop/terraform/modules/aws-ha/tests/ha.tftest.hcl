...
variables {
  ...
  stack_name    = "bridgefu-support-ha"
  ...
  parameters = {
    ...
    PrivateTlsSecretArn        = "arn:aws:secretsmanager:us-west-2:123456789012:secret:bridgefu-ha"
    ...
    RuntimeProfile             = "HighAvailability"
...
run "ha_contract" {
  ...
  assert {
    condition     = aws_cloudformation_stack.recipe.parameters["RuntimeProfile"] == "HighAvailability"
    error_message = "The module must select the HA root-template condition."
  }
