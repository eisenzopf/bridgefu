...
Only `recipe.yaml` is required by the runtime. The other files are recommended
for a distributable external recipe.
...
- `stable_uri`: fixed Request-URI user for a trusted, authenticated SIP peer.
  ...
  posture; both SIPS modes must use `managed_attachment`.
...
each fixed peer. Stable URI mode is not accepted for either SIPS posture
because CIDR identity is not equivalent to one-use admission.
