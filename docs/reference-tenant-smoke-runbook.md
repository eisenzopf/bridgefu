# Reference tenant non-production smoke, drain, and rollback
...
- The job enters the GitHub environment named exactly
  `reference-tenant-nonproduction`.
...
- The smoke runner verifies the exact allowlisted AWS account, an HTTPS
  ...
  and a Vapi payload marked `reference-tenant-nonproduction`.
...
|---|---|
| `REFERENCE_TENANT_SMOKE_AWS_ROLE_ARN` | OIDC role in the dedicated non-production AWS account |
| `REFERENCE_TENANT_SMOKE_AWS_REGION` | Region containing the non-production Connect instance |
| `REFERENCE_TENANT_SMOKE_AWS_ACCOUNT_ID` | Exact 12-digit account allowlist checked after OIDC |
| `REFERENCE_TENANT_SMOKE_BRIDGEFU_URL` | HTTPS API origin whose hostname visibly identifies staging/non-production |
| `REFERENCE_TENANT_SMOKE_DIAGNOSTICS_TOKEN` | Tenant-scoped bearer token for screen-pop evidence |
| `REFERENCE_TENANT_SMOKE_VAPI_TOKEN` | Token for the isolated Vapi test organization |
| `REFERENCE_TENANT_SMOKE_VAPI_CALL_PAYLOAD_JSON` | Complete call payload using only test assistant, number, and destination resources |
| `REFERENCE_TENANT_ROLLBACK_SSH_TARGET` | `user@host` for only the non-production Bridgefu host |
| `REFERENCE_TENANT_ROLLBACK_SSH_PRIVATE_KEY` | Least-privilege deploy key for that host |
| `REFERENCE_TENANT_ROLLBACK_SSH_KNOWN_HOSTS` | Pinned `known_hosts` entry collected through an authenticated channel |
| `REFERENCE_TENANT_ROLLBACK_IMAGE` | Pre-staged immutable `sha256:…` image ID or `repository@sha256:…` reference |
...
```json
{
  ...
  "assistantOverrides": {
    "variableValues": {
      "bridgefuEnvironment": "reference-tenant-nonproduction",
...
```bash
bash scripts/check-reference-tenant-smoke-artifacts.sh
```

Or dispatch `Reference tenant non-production smoke` with the default values:
...
```bash
bash scripts/reference-tenant-drain-rollback.sh plan
```
