...
|---|---|
| rvoip packages | Exact crates.io `=0.3.5` component releases; `Cargo.lock` records every registry checksum and transitive package |
| SIP/RTP | Audio, G.711 µ-law/A-law, RFC 4733 DTMF |
| WebRTC | Opus, WHIP/WHEP, WS/WSS signaling, arbitrary labeled DataChannels |
| WebRTC engine | crates.io `rvoip-webrtc = 0.3.5`, backed by `rvoip-webrtc-stack = 0.3.5` and `rvoip-rtc = 0.3.5`; no Git or path override |
...
| MOQT wire library | crates.io `rvoip-moq = 0.3.5` with exact `rvoip-moq-transport`, `rvoip-moq-native`, and `rvoip-moq-relay` 0.3.5 packages recorded in `Cargo.lock` |
...
lifecycle types. Release builds now select the checksummed crates.io 0.3.5
...
`rvoip-webrtc-stack` and `rvoip-rtc` 0.3.5 dependencies. It supports the
