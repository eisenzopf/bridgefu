    ...
    match admission.mode {
        ...
        SipAdmissionMode::StableUri => {
            ...
            if matches!(
                ...
                return invalid(format!(
                    "bridge {bridge_id:?} secure SIP posture requires managed_attachment admission"
                ));
            }
            Ok(())
        }
...
fn validate_sip_uri(value: &str, security: SipSecurity) -> Result<(), RecipeError> {
    ...
    let expected = match security {
        SipSecurity::SipsSrtp | SipSecurity::SipsOptionalSrtp => "sips:",
...
"#,
    ...
    #[test]
    fn flagship_recipe_compiles_tls_with_optional_srtp_as_an_explicit_posture() {
        let catalog = RecipeCatalog::builtin().unwrap();
        let package = catalog
            .resolve("builtin:vapi-amazon-connect-screen-pop@1")
            .unwrap();
        ...
        values.insert(
            "sip_security".into(),
            Value::String("sips_optional_srtp".into()),
        );

        let compiled = package.compile(&values).unwrap();
        let source = &compiled.spec.bridges.get("transfer").unwrap().source;
        let RecipeEndpointSpec::Sip {
            ...
            panic!("flagship source must remain managed SIP ingress");
        };
        assert_eq!(*security, SipSecurity::SipsOptionalSrtp);
        assert_eq!(admission.mode, SipAdmissionMode::ManagedAttachment);
    }
