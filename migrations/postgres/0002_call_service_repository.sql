...
UPDATE outbox
SET body = jsonb_set(body, '{state,claimed_at}', to_jsonb(available_at), TRUE)
WHERE outbox_state = 'claimed'
  AND body #>> '{state,state}' = 'claimed'
  AND NOT (body -> 'state' ? 'claimed_at');
...
SET body = jsonb_set(
    body,
    '{row,receipt}',
    '{"receipt":"create_call"}'::jsonb,
    TRUE
)
WHERE NOT (body -> 'row' ? 'receipt');
...
ALTER TABLE idempotency ALTER COLUMN receipt_kind DROP DEFAULT;
ALTER TABLE idempotency ALTER COLUMN operation_kind DROP DEFAULT;

ALTER TABLE repository_metadata
    DROP CONSTRAINT repository_metadata_schema_version_check;
UPDATE repository_metadata SET schema_version = 2 WHERE singleton = TRUE;
ALTER TABLE repository_metadata
    ADD CONSTRAINT repository_metadata_schema_version_check CHECK (schema_version = 2);
...
CREATE TABLE call_execution_plans (
    ...
    first_leg_id UUID NOT NULL,
    first_endpoint_kind TEXT NOT NULL,
    second_leg_id UUID NOT NULL,
...
CREATE TABLE service_effect_payloads (
    effect_id UUID PRIMARY KEY REFERENCES outbox(effect_id) ON DELETE CASCADE,
    command_id UUID NOT NULL REFERENCES service_command_results(command_id) ON DELETE CASCADE,
...
CREATE TABLE control_outbox (
    ...
    command_id UUID NOT NULL UNIQUE REFERENCES control_commands(command_id) ON DELETE CASCADE,
    tenant_id TEXT NOT NULL,
    call_id UUID NOT NULL REFERENCES call_execution_plans(call_id) ON DELETE CASCADE,
    leg_id UUID NOT NULL,
    binding_generation BIGINT NOT NULL CHECK (binding_generation > 0),
...
CREATE TABLE external_references (
    ...
    leg_id UUID NOT NULL,
    binding_generation BIGINT NOT NULL CHECK (binding_generation > 0),
    effect_id UUID NOT NULL UNIQUE REFERENCES outbox(effect_id) ON DELETE RESTRICT,
    bound_at TIMESTAMPTZ NOT NULL,
    body JSONB NOT NULL,
