...
readonly REQUIRED_ENVIRONMENT="standardcharter-nonproduction"
...
run_smoke() {
  ...
  if [[ -n "${GITHUB_STEP_SUMMARY:-}" ]]; then
    {
      echo "## StandardCharter non-production smoke"
      echo
      echo "Result: passed"
  ...
  trap - EXIT INT TERM
  cleanup
  echo "standardcharter smoke: redacted lifecycle qualification passed"
}

case "${1:-preflight}" in
  preflight)
    validate_configuration
    echo "standardcharter smoke: secret-backed configuration is valid; no network calls were made"
    ;;
  execute)
    validate_configuration
    run_smoke
